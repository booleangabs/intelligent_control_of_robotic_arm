...
with open('winner_genome_99_angle_real.pkl', 'rb') as f:
...
config = neat.Config(
    ...
    'config-feedforward.txt'  # esse é o arquivo de configuração
)
...
while True:
    ...
    if ids is not None:
        ...
        if 0 in marker_positions and 1 in marker_positions:
            ...
            angles = net.activate(Pt_screen)
            angles = [s * 180 for s in angles] + [0, 0]
            angles_next = np.ceil(map_theta(np.deg2rad(angles), initial_theta))
